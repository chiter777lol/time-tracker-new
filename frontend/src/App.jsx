...
function App() {
  ...
  const updateProject = async () => {
    ...
    try {
      await api.updateProject(editingProject._id, { name: editProjectName, description: editProjectDesc });
  ...
  const updateProfile = async () => {
    ...
    try {
      const updatedUser = { ...user, name: editProfileName, email: editProfileEmail };
    ...
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '1rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', borderBottom: '1px solid #e2e8f0', paddingBottom: '1rem' }}>
        <div style={{ display: 'flex', gap: '1rem' }}>
          ...
          <button onClick={() => setView('analytics')} style={{ background: view === 'analytics' ? '#4f46e5' : 'white', color: view === 'analytics' ? 'white' : '#4f46e5', padding: '0.5rem 1rem', borderRadius: '0.5rem', border: '1px solid #4f46e5', cursor: 'pointer' }}>Аналитика</button>
          <button onClick={() => { setIsEditingProfile(true); setEditProfileName(user.name); setEditProfileEmail(user.email); }} style={{ background: view === 'profile' ? '#4f46e5' : 'white', color: view === 'profile' ? 'white' : '#4f46e5', padding: '0.5rem 1rem', borderRadius: '0.5rem', border: '1px solid #4f46e5', cursor: 'pointer' }}>Профиль</button>
        </div>
      ...
      {isEditingProfile && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: 'white', padding: '2rem', borderRadius: '1rem', width: '400px' }}>
            <h3 style={{ marginBottom: '1rem' }}>Редактировать профиль</h3>
            <input type="text" placeholder="Имя" value={editProfileName} onChange={e => setEditProfileName(e.target.value)} style={{ width: '100%', padding: '0.5rem', marginBottom: '1rem', border: '1px solid #e2e8f0', borderRadius: '0.5rem' }} />
            <input type="email" placeholder="Email" value={editProfileEmail} onChange={e => setEditProfileEmail(e.target.value)} style={{ width: '100%', padding: '0.5rem', marginBottom: '1rem', border: '1px solid #e2e8f0', borderRadius: '0.5rem' }} />
            <div style={{ display: 'flex', gap: '1rem', justifyContent: 'flex-end' }}>
              <button onClick={() => setIsEditingProfile(false)} style={{ padding: '0.5rem 1rem', border: '1px solid #e2e8f0', borderRadius: '0.5rem', cursor: 'pointer' }}>Отмена</button>
              <button onClick={updateProfile} style={{ background: '#4f46e5', color: 'white', padding: '0.5rem 1rem', borderRadius: '0.5rem', border: 'none', cursor: 'pointer' }}>Сохранить</button>
            </div>
          </div>
        </div>
      )}
      ...
      {editingProject && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: 'white', padding: '2rem', borderRadius: '1rem', width: '400px' }}>
            <h3 style={{ marginBottom: '1rem' }}>Редактировать проект</h3>
            <input type="text" placeholder="Название" value={editProjectName} onChange={e => setEditProjectName(e.target.value)} style={{ width: '100%', padding: '0.5rem', marginBottom: '1rem', border: '1px solid #e2e8f0', borderRadius: '0.5rem' }} />
            <textarea placeholder="Описание" value={editProjectDesc} onChange={e => setEditProjectDesc(e.target.value)} style={{ width: '100%', padding: '0.5rem', marginBottom: '1rem', border: '1px solid #e2e8f0', borderRadius: '0.5rem' }} rows="3" />
            <div style={{ display: 'flex', gap: '1rem', justifyContent: 'flex-end' }}>
              <button onClick={() => setEditingProject(null)} style={{ padding: '0.5rem 1rem', border: '1px solid #e2e8f0', borderRadius: '0.5rem', cursor: 'pointer' }}>Отмена</button>
              <button onClick={updateProject} style={{ background: '#4f46e5', color: 'white', padding: '0.5rem 1rem', borderRadius: '0.5rem', border: 'none', cursor: 'pointer' }}>Сохранить</button>
            </div>
          </div>
        </div>
      )}
